...
class DataProcessor:
    ...
    def _parse_filter_name(self, filter_name: str) -> Tuple[str, str]:
        ...
        # Mapear tipos de faceta al estándar (español + portugués)
        type_mapping = {
            # Tamaño
            'pulgadas': 'size',
            'tamaño': 'size',
            'tamanho em polegadas': 'size',
            ...
            # Tecnología/Panel
            'tipo pantalla': 'technology',
            'tipo de painel': 'technology',
            ...
            'frequência de atualização': 'refresh_rate',
    ...
    def load_filter_usage_url(self, file_content: str, source_name: str = 'all') -> pd.DataFrame:
        ...
        for line in lines[data_start:]:
            ...
            if len(parts) == 2:
                ...
                # Solo URLs de la categoría
                if self.category_keyword not in url.lower():
                    continue
                
                # Extraer facetas de la URL
                url_info = self.classify_url(url)
                
                # Determinar tipo principal de faceta
                facet_type = 'category'
                facet_value = self.category_keyword
                
                if url_info['facets']:
                    # Tomar la primera faceta no nula
