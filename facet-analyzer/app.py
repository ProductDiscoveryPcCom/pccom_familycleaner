...
from utils import DataProcessor, FacetAnalyzer, IndexationAnalyzer, LLMValidator, AnalysisResults
...
def render_sidebar():
    with st.sidebar:
        ...
        filter_all_file = st.file_uploader("📈 Uso Filtros (Todo tráfico)", type=['csv'], key='fall')
        filter_seo_file = st.file_uploader("🌐 Uso Filtros (SEO)", type=['csv'], key='fseo')
    ...
    with st.spinner("Procesando datos..."):
        ...
        if files.get('filter_all_file'):
            try:
                ...
                loaded.append("Uso Filtros (Todo)")
            except Exception as e:
...
def render_ux_seo_tab():
    ...
    **Interpretación:**
    ...
    st.dataframe(
        display_df.style.background_gradient(subset=['Gap'], cmap='RdYlGn'),
