...
def render_insights_tab():
    ...
    # Mostrar fuentes de datos cargadas
    st.markdown("---")
    ...
    if data_sources:
        ...
        st.warning("⚠️ No hay datos cargados. Sube archivos en la barra lateral.")
        return
    
    # Métricas principales diferenciadas por fuente
    st.markdown("---")
    ...
    with col1:
        ...
        if total_sf > 0:
            st.metric("🔍 Search Filters", f"{total_sf:,}" if total_sf < 1000000 else f"{total_sf/1000000:.1f}M", 
                     help="Interacciones con filtros (faceta:valor)")
        else:
            st.metric("🔍 Search Filters", "No cargado")
    
    with col2:
        # Métricas de Page URL
        ...
        if total_url > 0:
            st.metric("🔗 Page Full URL", f"{total_url:,}" if total_url < 1000000 else f"{total_url/1000000:.1f}M",
                     help="Sesiones por URL (arquitectura N0-N3+)")
        else:
    ...
    with col3:
        ...
        st.metric("📈 Ratio SEO", f"{seo_ratio:.1f}%",
                 help="% del tráfico que viene de SEO")
    ...
    if insights:
        ...
        if high_priority:
            ...
            for insight in high_priority:
                source_badge = f"*[{insight.get('source', 'Unknown')}]*" if insight.get('source') else ''
        ...
        st.info("Carga más datos (Search Filters + Page Full URL) para generar insights automáticos")
    ...
    with col1:
        # Gráfico de facetas (desde Search Filters)
        ...
        if facet_usage:
            facet_df = pd.DataFrame([
                ...
                if k not in ['total', 'other', 'sorting', 'price', 'search filters'] and v.get('pct_all', 0) > 0.5
            ]).sort_values('Uso (%)', ascending=False).head(6)
            
            if not facet_df.empty:
                st.markdown("**Uso de Facetas** *(Search Filters)*")
                fig = px.pie(facet_df, values='Uso (%)', names='Faceta',
                            color_discrete_sequence=px.colors.qualitative.Set2)
                fig.update_layout(height=350)
                st.plotly_chart(fig, width="stretch")
        else:
            st.info("Carga 'Search Filters' para ver uso de facetas")
    
    with col2:
        # Gráfico de arquitectura (desde Page Full URL)
        architecture = insights_data.get('architecture', {})
        level_dist = architecture.get('level_distribution', [])
        if level_dist:
            level_df = pd.DataFrame(level_dist[:6])
            if not level_df.empty:
                st.markdown("**Distribución por Nivel** *(Page Full URL)*")
                level_df['Nivel'] = level_df['level'].apply(lambda x: f'N{x}')
                fig = px.bar(level_df, x='Nivel', y='sessions', 
                            color='pct',
                            color_continuous_scale='Viridis',
                            labels={'sessions': 'Sesiones', 'pct': '% del total'})
                fig.update_layout(height=350)
                st.plotly_chart(fig, width="stretch")
        else:
            st.info("Carga 'Page Full URL' para ver distribución por nivel")
    ...
    with col1:
        ...
        if brand_data:
            ...
            if not brand_df.empty and 'brand' in brand_df.columns:
                st.markdown("**Share por Marca: UX vs SEO** *(Search Filters)*")
    ...
    with col2:
        # Combinaciones N2 más usadas
        n2_analysis = architecture.get('n2_analysis', {})
        facet_orders = n2_analysis.get('facet_orders', [])
        if facet_orders:
            st.markdown("**Top Combinaciones N2** *(Page Full URL)*")
            combo_df = pd.DataFrame(facet_orders[:8])
            if not combo_df.empty:
                fig = px.bar(combo_df, x='order', y='sessions',
                            labels={'order': 'Combinación', 'sessions': 'Sesiones'},
                            color='sessions',
    ...
    <div style="background: rgba(0,217,255,0.1); border: 1px solid rgba(0,217,255,0.3); border-radius: 10px; padding: 1rem; margin-top: 1rem;">
        <h4>💡 Sobre las Fuentes de Datos</h4>
        <ul>
            <li><strong>Search Filters:</strong> Qué facetas usan los usuarios (pulgadas:55, marcas:lg...)</li>
            <li><strong>Page Full URL:</strong> Arquitectura de URLs navegadas (N0, N1, N2, N3+)</li>
            <li><strong>Top Query:</strong> Rendimiento SEO y detección de canibalización</li>
        </ul>
        <p>Para un análisis completo, carga <strong>los 4 archivos de Adobe Analytics</strong> + Top Query.</p>
    </div>
    """, unsafe_allow_html=True)
